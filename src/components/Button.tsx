...
export default function Button({ type = '', text }: ButtonProps) {
  function renderStyle(type: string) {
    switch (type) {
      ...
      default:
        return 'bg-pr-dark-purple desktop:py-3.5 desktop:px-8 hover:bg-pr-grayish-blue py-[11px] px-[24px]';
    }
  }
  return (
    <>
      <button
        className={`capitalize relative overflow-hidden transition-all px-8 font-bold text-[18px] leading-[28px] text-white rounded-full ${renderStyle(
          type
        )}`}
      >
        {text}
